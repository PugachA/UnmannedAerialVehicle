...
void PIReg::calcGainParams(double air_speed)
{
	uint8_t i = 0;
	double k_pr_buf = 0.0;
	double k_int_buf = 0.0;

	if (air_speed <= this->speed_ref_points[0]) //minimum
		{
			k_pr_buf = this->k_pr_points[0];
			k_int_buf = this->k_int_points[0];
		}
		else
			if (air_speed >= this->speed_ref_points[this->num_of_ref_points-1]) //maximum
			{
